...
pub fn all_body_update(sim: &mut Simulator) {
    for i in 0..sim.bodies.len() {
        ...
        for j in 0..sim.bodies.len() {
            ...
            let numerator = (G_CONSTANT * (sim.bodies[i].radius as f64) * (sim.bodies[j].radius as f64))/(dist + (DAMPENING_FACTOR as f64)).powi(2);
            for dim in 0..DIMENSIONS {
                dV[dim] = numerator * unit_vec[dim];
            }
        }

        for dim in 0..DIMENSIONS {
            sim.bodies[i].vel[dim] += dV[dim];
        }
    }
}